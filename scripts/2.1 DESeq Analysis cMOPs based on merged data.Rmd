---
title: "2.1 Re-analysis of Transgen - cMOPs"
...
# subset on cMOP samples
sample_table <- sample_table[sample_table$Celltype == "cMOP", ]

sample_table$condition <- factor(sample_table$condition,
                                 levels = c("cMOP_control", "cMOP_infected"))
...
#                                     design = ~condition)
# saveRDS(dds_txi, "dds_txi_cMOPs.rds")
dds_txi <- readRDS("dds_txi_cMOPs.rds")
```
...
```{r}
table(c(DEGs_publication$CMOP_down[1:6], DEGs_publication$CMOP_up[1:6]) %in%norm_anno$SYMBOL )

c(DEGs_publication$CMOP_down[1:6], 
  DEGs_publication$CMOP_up[1:6])[c(DEGs_publication$CMOP_down[1:6], DEGs_publication$CMOP_up[1:6]) %in%norm_anno$SYMBOL]
```
...
```{r}
comparison_table <- data.frame(comparison = c("cMOP_infected"),
                               control = c("cMOP_control"))
```


## 7.1 Perform Differential Expression Testing
...
```{r}
# DEGs_cMOPs_TPM2_nobatch <- 
#   DEresults$cMOP_infected_vs_cMOP_control@results[DEresults$cMOP_infected_vs_cMOP_control@results$regulation != "n.s.", ]

# DEGs_cMOPs_TPM2_batch <-
#   DEresults$cMOP_infected_vs_cMOP_control@results[DEresults$cMOP_infected_vs_cMOP_control@results$regulation != "n.s.", ]

# DEGs_cMOPs_count10_nobatch <-
#   DEresults$cMOP_infected_vs_cMOP_control@results[DEresults$cMOP_infected_vs_cMOP_control@results$regulation != "n.s.", ]

# DEGs_cMOPs_count10_batchcorr <-
#   DEresults$cMOP_infected_vs_cMOP_control@results[DEresults$cMOP_infected_vs_cMOP_control@results$regulation != "n.s.", ]

# DEGs_cMOPs_count10_samples5_nobatch <-
#   DEresults$cMOP_infected_vs_cMOP_control@results[DEresults$cMOP_infected_vs_cMOP_control@results$regulation != "n.s.", ]

# DEGs_cMOPs_count10_samples5_batchcorr <-
#   DEresults$cMOP_infected_vs_cMOP_control@results[DEresults$cMOP_infected_vs_cMOP_control@results$regulation != "n.s.", ]
```
...
```{r, echo=TRUE, message=FALSE, results='hide', fig.height=6, fig.width=6}
...
tmp <- tmp[, grepl(pattern = "cMOP", x = colnames(norm_anno))] %>% rownames_to_column("gene")
...
```{r}
table(DEresults_list$cMOP_infected_vs_cMOP_control@results[DEresults_list$cMOP_infected_vs_cMOP_control@results$regulation != "n.s.",]$SYMBOL %in%
        c(DEGs_publication$CMOP_down, DEGs_publication$CMOP_up))
```

```{r, fig.height=4, fig.width=5}
res <- DEresults_list$cMOP_infected_vs_cMOP_control@results
genes <-  res[res$regulation != "n.s." & res$SYMBOL %in% c(DEGs_publication$CMOP_down, DEGs_publication$CMOP_up), ]$SYMBOL
